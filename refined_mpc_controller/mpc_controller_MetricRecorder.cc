...
Status MPCController::ComputeControlCommand(
  ...
  // Query with 0.8s delay (as the lateral control module did)
  //const auto NextStateChangePoint_time = trajectory_analyzer_.QueryNearestPointByAbsoluteTime(injector_->vehicle_state()->timestamp() + 0.8);
  
  // Query with just this module did.
  const auto NextStateChangePoint_time = trajectory_analyzer_.QueryNearestPointByAbsoluteTime(injector_->vehicle_state()->timestamp());
  //const auto NextStateChangePoint_position = trajectory_analyzer_.QueryNearestPointByPosition(injector_->vehicle_state()->x(), injector_->vehicle_state()->y());
  
  //const auto NextStateChangePoint = trajectory_analyzer_.QueryNearestPointByAbsoluteTime(vehicle_state_mount_time->timestamp() + 0.05);
  const double plan_header_time = trajectory_analyzer_.header_time_;
  //const auto queryTime = NextStateChangePoint_time.relative_time() + plan_header_time;


  ofstream file("recorded_two_trajectories.txt", ios::app);
  file << std::fixed;
  file << "Planning header time: " << plan_header_time << "\n***\n";

  file << "Planning point (by time) timestamp: " << plan_header_time + NextStateChangePoint_time.relative_time() << "\n";
  file << "Planning point (by time) x: " << NextStateChangePoint_time.path_point().x() << "\n";
  file << "Planning point (by time) y: " << NextStateChangePoint_time.path_point().y() << "\n";
  file << "Planning point (by time) theta: " << NextStateChangePoint_time.path_point().theta() << "\n";
  file << "Planning point (by time) kappa: " << NextStateChangePoint_time.path_point().kappa() << "\n";
  file << "Planning point (by time) v: " << NextStateChangePoint_time.v() << "\n";
  file << "Planning point (by time) a: " << NextStateChangePoint_time.a() << "\n***\n";

  file << "Current point timestamp: " << vehicle_state_mount_time->timestamp() << "\n";
  file << "Current point x: " << vehicle_state_mount_time->x() << "\n";
  file << "Current point y: " << vehicle_state_mount_time->y() << "\n";
  file << "Current point theta: " << vehicle_state_mount_time->heading() << "\n";
  file << "Current point kappa: " << vehicle_state_mount_time->kappa() << "\n";
  file << "Current point v: " << vehicle_state_mount_time->linear_velocity() << "\n";
  file << "Current point a: " << vehicle_state_mount_time->linear_acceleration() << "\n-------------------------------\n";
