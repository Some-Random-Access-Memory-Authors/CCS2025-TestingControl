...
class TestScenario:
    ...
    def add_autopilot_vehicle(self, actor_type, nav_type, speed, sp_transform, dp_transform):

        spawn_point = (
            (sp_transform.location.x, sp_transform.location.y, sp_transform.location.z),
            (sp_transform.rotation.roll, sp_transform.rotation.pitch, sp_transform.rotation.yaw)
        )

        dest_point = (
            (dp_transform.location.x, dp_transform.location.y, dp_transform.location.z),
            (dp_transform.rotation.roll, dp_transform.rotation.pitch, dp_transform.rotation.yaw)
        )
    ...
    def run_test(self, state):
        ...
        ################ New Oracles Below
        diff_angle_list = np.array(state.diff_angle_list)
        diff_distance_list = np.array(state.diff_distance_list)
        
        avg_diff_angle = np.mean(diff_angle_list)
        avg_diff_distance = np.mean(diff_distance_list)

        acc_abs_list = np.array( [abs(x) for x in np.diff(speed_list)] )
        acc_lat_list = np.array( [abs(x) for x in np.diff(Vy_list)] )
        acc_lon_list = np.array( [abs(x) for x in np.diff(Vx_list)] )
        steer_acc_list = np.array( [abs(x) for x in np.diff(SWA_list)] )
        steer_yaw_list = np.array( [abs(x) for x in np.diff(np.array(state.yaw_rate_list))] )

        var_acc_abs = np.var(acc_abs_list)
        var_acc_lat = np.var(acc_lat_list)
        var_acc_lon = np.var(acc_lon_list)
        var_acc_steer = np.var(steer_acc_list)
        var_acc_yaw = np.var(steer_yaw_list)
        ...
        print("[debug] # oversteer:", num_oversteer)
        print("[debug] # understeer:", num_understeer)
        print("[debug] ### plan angle diff:", avg_diff_angle)
        print("[debug] ### plan distance diff:", avg_diff_distance)
        print("[debug] ### var_acc_abs:", var_acc_abs)
        print("[debug] ### var_acc_lat:", var_acc_lat)
        print("[debug] ### var_acc_lon:", var_acc_lon)
        print("[debug] ### var_acc_steer:", var_acc_steer)
        print("[debug] ### var_acc_yaw:", var_acc_yaw)

        deductions += 10*avg_diff_angle + 100*avg_diff_distance + 10*var_acc_abs + 10*var_acc_steer + 10*var_acc_yaw
