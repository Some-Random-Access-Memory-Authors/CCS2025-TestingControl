...
def get_nearby_spawnpoints(spawn_points, reference_location, distance_threshold):
    """
    Get spawn points that are within a specified distance from a reference location.

    :param spawn_points: A list of carla.Transform objects.
    :param reference_location: A carla.Location object.
    :param distance_threshold: A distance value in meters.
    :return: A list of spawn points that are within the specified distance from the reference location.
    """
    
    nearby_spawnpoints = [spawn_point for spawn_point in spawn_points if spawn_point.location.distance(reference_location) < distance_threshold]
    return nearby_spawnpoints


def filter_spawnpoints(spawn_points, location_to_exclude, distance_threshold):
    """
    Get spawn points that are within a specified distance from a reference location.
    ...
    #    print('dis: ', spawn_point.location.distance(location_to_exclude))
    
    filtered_spawnpoints = [spawn_point for spawn_point in spawn_points if spawn_point.location.distance(location_to_exclude) > distance_threshold]
    return filtered_spawnpoints
...
def main():
    ...
    while True:
        ...
        try:
            ...
            print("Num of spawn points in town: ", len(spawn_points))
            ...
            while math.sqrt((sp_x - wp_x) ** 2 + (sp_y - wp_y) ** 2) > 50:
                wp = random.choice(spawn_points)
                wp_x = wp.location.x
                wp_y = wp.location.y
                wp_z = wp.location.z
        ...
        seed_json = test_scenario.seed_data
        print("Using Initial Seed: ", seed_json)


        # Get npc vehicle from json.
        # I plan to only include Autopilot for now.
        npc_vehicles_json = seed_json.get('vehicle list', [])
        print('parsed auto vehicles: ', npc_vehicles_json)
        linear_vehicles_json = seed_json.get('linear vehicle list', [])
        print('parsed linear vehicles: ', linear_vehicles_json)
        static_vehicles_json = seed_json.get('static vehicle list', [])
        print('parsed static vehicles: ', static_vehicles_json)

        spawn_x = seed_json.get('sp_x')
        spawn_y = seed_json.get('sp_y')
        spawn_z = seed_json.get('sp_z')

        adas_spawn_location = carla.Location(x=spawn_x, y=spawn_y, z=spawn_z)
        print('adas spawn location: ', adas_spawn_location)
        ...
        while cycle_cnt < conf.max_cycles:
            ...
            client.set_timeout(20)
            town_map = "Town0{}".format(conf.town)
            print("Current town: ", town_map)
            client.load_world(town_map)
            world = client.get_world()
            town_from_world = world.get_map()
            
            #spawn_points_for_query = town_from_world.get_spawn_points()
            #print("Num of spawn points in town: ", len(spawn_points_for_query))

            current_map = world.get_map()
            print("current map: ", current_map)
            generated_waypoints = current_map.generate_waypoints(distance=2)
            print("num of waypoints: ", len(generated_waypoints))
            generated_transforms = [waypoint.transform for waypoint in generated_waypoints]
            print('num of transforms: ', len(generated_transforms))
            spawn_points_for_query = generated_transforms
            ...
            while round_cnt < conf.max_mutations: # mutation rounds
                ...
                ### Add autopilot vehicles!
                for npc_vehicle in npc_vehicles_json:
                    start = npc_vehicle['start']
                    end = npc_vehicle['end']

                    start_location = carla.Location(x=start['x'], y=start['y'], z=start['z'])
                    end_location = carla.Location(x=end['x'], y=end['y'], z=end['z'])

                    spawnpoints_near_start = get_nearby_spawnpoints(spawn_points_for_query, start_location, distance_threshold=15)
                    spawnpoints_near_end = get_nearby_spawnpoints(spawn_points_for_query, end_location, distance_threshold=40)

                    spawnpoints_near_start_filtered = filter_spawnpoints(spawnpoints_near_start, adas_spawn_location, distance_threshold=3)


                    mutated_transform_start = random.choice(spawnpoints_near_start_filtered)
                    mutated_transform_end = random.choice(spawnpoints_near_end)

                    mutated_transform_start.location.z = 0.3


                    print("Autopilot After initial filtering: ", len(spawnpoints_near_start), len(spawnpoints_near_end))

                    print("Autopilot After final filtering: ", len(spawnpoints_near_start_filtered), len(spawnpoints_near_end))

                    ret = test_scenario_m.add_autopilot_vehicle(actor_type=c.VEHICLE,
                                    nav_type=c.AUTOPILOT, speed=None, sp_transform=mutated_transform_start, dp_transform=mutated_transform_end)
                    print("Autopilot Vehicle Added: ", mutated_transform_start, mutated_transform_end)
                                    

                #print("linear!!!!!!!: ", linear_vehicles_json)
                for linear_npc_vehicle in linear_vehicles_json:
                    start = linear_npc_vehicle['start']
                    #end = linear_npc_vehicle['end']
                    end = linear_npc_vehicle['start']
                    ...
                    #start_x = random.uniform(start['x']-deviate_range, start['x']+deviate_range)
                    #start_y = random.uniform(start['y']-deviate_range, start['y']+deviate_range)

                    #end_x = random.uniform(end['x']-deviate_range, end['x']+deviate_range)
                    #end_y = random.uniform(end['y']-deviate_range, end['y']+deviate_range)

                    mutated_transform_start = random.choice(spawnpoints_near_start_filtered)
                    mutated_transform_end = random.choice(spawnpoints_near_end)

                    #mutated_transform_start.location.x = start_x
                    #mutated_transform_start.location.y = start_y
                    

                    #mutated_transform_end.location.x = end_x
                    #mutated_transform_end.location.y = end_y


                    mutated_transform_start.location.z = 0.3

                    #print("Linear After initial filtering: ", len(spawnpoints_near_start), len(spawnpoints_near_end))
                    #print("Linear After final filtering: ", len(spawnpoints_near_start_filtered), len(spawnpoints_near_end))

                    ret = test_scenario_m.add_autopilot_vehicle(actor_type=c.VEHICLE,
                                    nav_type=c.LINEAR, speed=15, sp_transform=mutated_transform_start, dp_transform=mutated_transform_end)
                    print("Linear Vehicle Added: ", mutated_transform_start, mutated_transform_end)


                for npc_vehicle in static_vehicles_json:
                    start = npc_vehicle['start']
                    end = npc_vehicle['end']
                    ...
                    spawnpoints_near_start = get_nearby_spawnpoints(spawn_points_for_query, start_location, distance_threshold=50)
                    spawnpoints_near_end = get_nearby_spawnpoints(spawn_points_for_query, end_location, distance_threshold=10)
                    ...
                    print("Static After initial filtering: ", len(spawnpoints_near_start), len(spawnpoints_near_end))
                    print("Static After final filtering: ", len(spawnpoints_near_start_filtered), len(spawnpoints_near_end))

                    ret = test_scenario_m.add_autopilot_vehicle(actor_type=c.VEHICLE,
                                    nav_type=c.IMMOBILE, speed=None, sp_transform=mutated_transform_start, dp_transform=mutated_transform_end)
                    print("Static Vehicle Added: ", mutated_transform_start, mutated_transform_end)
                ...
                print("Driving quality score: ", test_scenario_m.driving_quality_score)
