...
def return_angle_diff(player_transform, anchor_waypoint):
    angle_diff =  abs(player_transform.rotation.yaw - anchor_waypoint.transform.rotation.yaw)
    if angle_diff > 180:
...
def return_distance_diff(player_transform, anchor_waypoint):
    p = [player_transform.location.x, player_transform.location.y]
    q = [anchor_waypoint.transform.location.x, anchor_waypoint.transform.location.y]
...
def simulate(conf, state, town, sp, wp, weather_dict, frictions_list, actors_list):
    ...
    try:
        ...
        if conf.agent_type == c.BASIC:
            ...
            print("[+] spawned aggressive BehaviorAgent")
        ...
        try:
            ...
            while True:
                ...
                # Location, new param!
                anchor_waypoint = town_map.get_waypoint(player_loc, project_to_road=True)
                #print('anchor point: ', anchor_waypoint)
                #print('current point location: ', player_transform)
                

                diff_heading_angle = return_angle_diff(player_transform, anchor_waypoint)
                diff_distance = return_distance_diff(player_transform, anchor_waypoint)
                #print('diff angle: ', diff_heading_angle)
                #print('diff distance: ', diff_distance)

                #print('-')


                state.diff_angle_list.append(diff_heading_angle)
                state.diff_distance_list.append(diff_distance)
                ...
                if conf.agent_type == c.BASIC:
                    ...
                    agent.update_information(DummyWorld.DummyWorld(world, player))
        ...
        # I still have to comment this line.
